...
### 功能需求
- 选中文本/图片后，通过命令添加到指定 Canvas
- 支持文本、图片、公式三类节点，并根据类型使用不同尺寸策略
- 为节点记录 fromLink，点击节点可跳转回源文件选区
- 自动为有子节点的节点添加折叠按钮，支持多级折叠/展开
- 折叠/展开后自动重排可见节点，隐藏子树及相关边
- 删除节点时提供确认对话框，支持单节点删除与级联删除
- 删除边时提供确认对话框，并在失去入边时标记浮动节点
- 浮动节点以红框提示，重新连接后自动清除浮动状态
- 提供命令入口：新增节点、布局整理、删除边、批量调整高度
- 设置页支持配置节点尺寸、间距、折叠按钮颜色与调试开关
